...
""", unsafe_allow_html=True)
    ...
    def calcular_inicio_multa(data_despacho, prazo_dias, dias_uteis=False):
        """Calcula a data de início da multa com base no despacho e prazo"""
...
if "modo_entrada_faixa" not in st.session_state:
    st.session_state.modo_entrada_faixa = "Definir data final"

if "num_dias_faixa" not in st.session_state:
    st.session_state.num_dias_faixa = 5
...
with st.form("nova_faixa", clear_on_submit=True):
    ...
    modo_entrada = st.radio(
        ...
        horizontal=True,
        index=0 if st.session_state.modo_entrada_faixa == "Definir data final" else 1,
        help="Escolha entre informar a data final diretamente ou calcular baseado no número de dias",
        key="modo_entrada_radio"
    )
    
    # Atualiza o session state com a escolha atual
    st.session_state.modo_entrada_faixa = modo_entrada
    ...
    with col2:
        if st.session_state.modo_entrada_faixa == "Definir data final":
            ...
            num_dias = st.number_input(
                ...
                min_value=1,
                max_value=365,
                value=st.session_state.num_dias_faixa,
                step=1,
                help="Número de dias para a faixa",
                key="num_dias_input"
            )
            # Atualiza o session state com o número de dias
            st.session_state.num_dias_faixa = num_dias
    ...
    if st.session_state.modo_entrada_faixa == "Definir número de dias":
        data_fim = calcular_data_final(data_inicio, st.session_state.num_dias_faixa, tipo_dias == "Dias úteis")
    ...
    if submitted:
        if st.session_state.modo_entrada_faixa == "Definir número de dias":
            # Recalcula data_fim para garantir que está atualizada
            data_fim = calcular_data_final(data_inicio, st.session_state.num_dias_faixa, tipo_dias == "Dias úteis")
